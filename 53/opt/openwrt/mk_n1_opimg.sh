...
# 目标镜像文件
TGT_IMG="${WORK_DIR}/N1_53_Openwrt.img"
...
# 20210130 add
UBOOT_WITH_FIP="${PWD}/files/meson_btld/with_fip/u-boot-n1.bin"
...
sudo mkdir $LINUX_BOOT $LINUX_ROOT
...
echo "创建磁盘分区和文件系统 ..."
sudo parted -s $TGT_DEV mklabel msdos
BEGIN=$((SKIP_MB * 1024 * 1024))
END=$(( BOOT_MB * 1024 * 1024 + BEGIN -1))
sudo parted -s $TGT_DEV mkpart primary fat32 ${BEGIN}b ${END}b
if [ $? -ne 0 ];then
    echo "创建 boot 分区失败!"
    sudo losetup -D
    exit 1
...
sudo parted -s $TGT_DEV mkpart primary btrfs ${BEGIN}b 100%
if [ $? -ne 0 ];then
    echo "创建 rootfs 分区失败!"
    sudo losetup -D
    exit 1
...
( sudo losetup -D && cd $WORK_DIR && rm -rf $TEMP_DIR && sudo losetup -D)
sudo sync
echo
echo "镜像打包已完成，再见!"
